    ...
    <div class="card">
        ...
        <form action="{{ route('user.store') }}" method="POST">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-6 form-group"><label>Password</label><input type="password" name="password"
                            class="form-control @error('password') is-invalid @enderror" required>
                        @error('password')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-6 form-group"><label>Konfirmasi Password</label><input type="password"
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 form-group"><label>Gender</label><select name="id_gender" class="form-control"
                            required>
                            <option value="1" {{ old('id_gender') == 1 ? 'selected' : '' }}>Laki-laki</option>
                            <option value="2" {{ old('id_gender') == 2 ? 'selected' : '' }}>Perempuan</option>
                        </select></div>
            ...
            <div class="card-footer"><a href="{{ route('user.index') }}" class="btn btn-secondary">Batal</a><button
                    type="submit" class="btn btn-primary">Simpan</button></div>
        </form>
    </div>
@endsection
